/**
 * Logger interface for the Fuse SDK.
 ...
 * All logs are prefixed with [fuse-sdk] for easy identification.
 *
  ...
  return {
    debug: (message, meta) => {
      if (levels.debug >= minLevelValue) {
        ...
        console.debug(`[fuse-sdk] DEBUG: ${message}`, metaStr);
      }
    },
    error: (message, meta) => {
      if (levels.error >= minLevelValue) {
        const metaStr = formatMeta(meta);
        // eslint-disable-next-line no-console
        console.error(`[fuse-sdk] ERROR: ${message}`, metaStr);
      }
    },
    info: (message, meta) => {
      if (levels.info >= minLevelValue) {
        const metaStr = formatMeta(meta);
        // eslint-disable-next-line no-console
        console.info(`[fuse-sdk] INFO: ${message}`, metaStr);
      }
    },
    warn: (message, meta) => {
      if (levels.warn >= minLevelValue) {
        const metaStr = formatMeta(meta);
        // eslint-disable-next-line no-console
        console.warn(`[fuse-sdk] WARN: ${message}`, metaStr);
      }
    },
  };
};
